 ...
 **코로나 확진자 추이와 음식 배달 주문 데이터를 분석하여 요식업 (예비)창업자에게 도움이 될 수 있는 창업위치선정, 업종선정에 대한 인사이트를 도출하고, 머신러닝 모델을 활용하여 원가절감 및 수익 극대화 방안을 제공합니다.**   



### :two: 필요성 

 **포스트 코로나 시대가 도래한 지금, 소비자들의 라이프스타일과 소비 패턴이 변화하여 배달음식 시장은 눈에 띄게 성장하였으나, 영세한 자영업자의 영업이익 개선에는 큰 도움이 안되고 있습니다. 따라서 코로나 확진자 추이와 음식 배달 주문 데이터를 분석하여 요식업 (예비)창업자에게 도움을 주기 위해 프로젝트를 기획하였습니다.**
...
- merge 결과(월별 주문건수 및 누적확진자 데이터로 가공 완료)
...
## 필요 기간별 인구 데이터를 모두 불러온 후 concatenation 실행
```
...
```python
ex) a = df_pop2.iloc[:,4:18].sum(axis=1) ## 해당 코드를 반복적으로 실행
```

![pop2](md-images/pop2.PNG)

- [date ,행정동코드, 총생활인구수,	men,	women,	0~9,	10~19,	20~29,	30~39,	40~49,	50~59,	60~69,	70~] 으로 컬럼을 분류하여 병합
...
```python
## date(날짜)와 code(행정동코드)의 type을 맞춘 뒤 두 개의 컬럼을 기준으로 조인
df_fin = df.merge(df3, on=['date','code'], how='left')
```

![merge2](md-images/merge2.PNG)

- 날짜별 행정동 코드별로 정확히 구분하기 위해 이중조건으로 조인

  주문 건수 및 위치별 생활인구 수 날짜별 조회 가능
...
- 배달 주문데이터의 행정동 코드를 포함한 배달 상점 데이터 활용
  ...
  del_shop = pd.concat([delshop,delshop2,delshop3,delshop4,delshop5,delshop6,delshop7,delshop8,delshop9]) ## 모든 데이터 통합
  ```
...
- 해당 지역의 상점수(공급) 대비 배달 건수(수요)를 확인하여 점수로 나타내기 위한 전처리 작업 수행

![rel1_1](md-images/rel1_1.PNG)

- 태블로에서 릴레이션을 형성하여 작업
  - 태블로 내에는 조인과 다르게 간단히 두 컬럼을 비교하여 연결시켜주는 릴레이션 기능이 존재합니다. 위 이미지는 배달 상점 데이터와 배달 주문 데이터의 `Unique` 값이 같은 컬럼인 `code` , `menu`를 릴레이션 형성하는 과정입니다.

- 해당 칼럼을 함수로 계산하여 정규화 및 점수 산출

  <img src="md-images/rel2.png" alt="rel2" style="zoom:75%;" />

- 총 매출 금액을 로그화 하여 범위 한정하고 상점의 개수로 나누어 정규화 
...
- ARIMA모델(위)과  Facebook의  fbprophet모델(아래)을 활용하여 분석한 결과, 높은 적합도를 보인 fbprophet을 채택하였습니다.
...
- fbprophet은 ds 컬럼과 y컬럼을 가져야하기에 컬럼명을 변경하였습니다.
...
##### :point_right: 연속형 변수와 범주형 변수를 모두 사용할 수 있는 K-Modes의  K-Prototypes를 활용하여 비지도학습 군집분석을 수행하였습니다.
...
- n_init : 모델 생성 횟수(기본 값이 10이며, 자동으로 최적의 모델을 선정)
...
- 학습된 모델을 피클로 만들어 서버에 저장

- 피클을 통해 모델링 과정을 생략 함으로써 클라이언트 요청을 빠른 속도로 처리
...
- 통계치를 통해 군집별 특성을 파악하고, 위와 같이 특성에 따른 컨설팅을 제공
...
- 다양한 데이터 간 릴레이션을 형성하고 시각화 할 수 있어, 다수의 데이터와 컬럼에서 값을 추출하는 작업에 효율적인 Tableau를 데이터 시각화 툴로 선정하여 작업하였습니다.
...
- 코로나 발생 전후로 배달 주문이 크게 상승한것을 확인 할 수 있습니다.
...
- 시간대(아침 점심 저녁 야간)별로 어떤 업종의 배달 주문이 많이 들어왔는지 또한 어떻게 변화하였는지 한눈에 보기 쉽게 하기 위하여 그래프를 구성하였습니다.
...
- 시군구별 어떠한 종목이 어떻게 변화했는지 알 수 있습니다.

![gragh2_3](md-images/gragh2_3.png)

- 종목별 차지하는 비율의 변화도 한눈에 보일 수 있도록 구성하였습니다.

![gragh2_4](md-images/gragh2_4.png)

- 한눈에 보기 쉽게 하이라이트를 삽입하여 구성하였습니다.
...
- 위치별 매출액
  ...
  - 위치별로 마우스를 올리면 매출액을 확인할 수 있습니다.

- 구역별 업종분포

  - 해당 구역의 업종별 매출 분포를 확인할 수 있습니다.

- 업종별 랭킹

  -  업종별 매출 랭킹 상위 5가지를 한눈에 볼 수 있습니다.

- 성비

  - 해당 지역의 성비 및 인구수를 확인할 수 있습니다. 

- 나이분포

  - 해당 지역의 나이별 인구 분포를 확인할 수 있습니다.

  ![gragh4_2](md-images/gragh4_2.png)

- 특정 시/군/구 별 데이터를 검색하여 해당 지역별 데이터를 확인할 수 있도록 하였습니다.

  ![gragh4_3](md-images/gragh4_3.png)

- 더욱 세분화 된 창업 지역 선정을 위하여 서울시의 읍면동 데이터 시각화 하였습니다.

  ![gragh4_4](md-images/gragh4_4.png)

- 상단의 위치 찾기를 통하여 위치를 찾을 수 있도록 구성하였습니다.
...
- 창업 위치 선정에 이어 업종 선정에 도움을 주는 서비스

  - 선정한 위치를 검색하고 종목별로 데이터 및 창업 추천 점수를 확인할 수 있도록 구성하였습니다.

![gragh5_1](md-images/gragh5_1.png)

- 위치, 업종별로 대시보드의 제목이 변경되도록 구성하였습니다.

  ![gragh5_2](md-images/gragh5_2.png)

- 창업 SCORE / 코멘트

  - 창업 스코어 : 배달 상점 데이터(공급량) 대비 배달 주문 데이터(주문량 = 수요) 를 정규화 하여 산출해낸 점수입니다. 자세한 내용은 전처리 과정을 참고하시면 됩니다.
  - 코멘트 : 해당 점수에 맞는 코멘트를 달아 줄 수 있도록 `if` 문을 사용하여 점수별 코멘트를 작성하여 구성하였습니다.

- No.1매출/ 건당 평균 매출

  - 지역별 최고 매출 종목 및 선택한 종목의 건당 평균 매출을 확인할 수 있습니다.

- 업종별 랭킹

  - 지역별로 가장 매출이 높은 종목 3가지를 확인할 수 있습니다.

![gragh5_3](md-images/gragh5_3.png)

- 지역별 총 매출

  - 지역별로 총 매출량의 규모를 확인할 수 있습니다.

- 시간대별 매출 추이 

  - 4가지 시간대(아침, 점심, 저녁, 야간)로 분류된 데이터를 통하여 시간대별로 매출을 확인할 수 있습니다.

- 요일별 매출 그래프

  - 해당 지역의 선택한 종목의 요일별 매출 그래프를 확인할 수 있습니다. 
  - 가장 매출이 높은 날과 낮은 날을 한눈에 볼 수 있도록 구성하였고 코멘트까지 확인할 수 있습니다.
...
- 로그인 완료 - 본인이 설정한 이름으로 메뉴바에 사용자명 (하단에 drop down도 볼수있도록 구성.)
